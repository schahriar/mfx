...
export class Compositor extends MFXTransformStream {
    ...
    drained = false;
    constructor(pipeline, canvas = document.createElement("canvas")) {
        ...
        let textureIn;
        let textureOut;
        const attachTextureToFramebuffer = (texture) => {
        ...
        super({
            transform: async (frame, controller) => {
                ...
                await Promise.all(Object.keys(programInfos).map(async (programId, i) => {
                    ...
                    if (layerFrame.value) {
                        ...
                        gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, gl.RGBA, gl.UNSIGNED_BYTE, layerFrame.value);
