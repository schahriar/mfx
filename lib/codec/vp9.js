...
export const vp9 = {
    autoSelectCodec({ width, height, bitrate, bitDepth, profile, }) {
        const lumaPictureSize = width * height;
        const selectedProfile = profile || bitDepth < 10 ? "profile0" : "profile2";
        for (const level of levels) {
            if (lumaPictureSize <= level.maxLumaPictureSize &&
        ...
        throw new Error("No suitable profile and level found for the given parameters.");
    },
};
